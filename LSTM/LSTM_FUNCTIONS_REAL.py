...
# # LSTM model
# def build_model_LSTM(NEURONS, OPTIMIZER, LEARNING_RATE, NUM_LAYERS, DROPOUT, ACTIVATION, LOSS, lag):
#     model = Sequential()

#     model.add(LSTM(units=NEURONS, input_shape = (lag, 3), return_sequences=True)) 
...
# def hyperparameter_tunning(scaled_values_X, scaled_values_Y, lookback_bars, validation_bars, testing_bars, epochs, config, stock_name, patience, lag):    

#     # hyperparameter tunning
#     mse_best_model_train = {}
#     best_indexes = {}

#     ranges = list(range(lookback_bars, len(scaled_values_X) - testing_bars, validation_bars))
#     for i in range(0, len(ranges)):
#         train_X, train_y = scaled_values_X[ranges[i]-lookback_bars:ranges[i]], scaled_values_Y[ranges[i]-lookback_bars:ranges[i]]
#         val_X, val_Y = scaled_values_X[ranges[i]:ranges[i]+validation_bars], scaled_values_Y[ranges[i]:ranges[i]+validation_bars]
#         test_X, test_y = scaled_values_X[ranges[i]+validation_bars:ranges[i]+validation_bars+testing_bars], scaled_values_Y[ranges[i]+validation_bars:ranges[i]+validation_bars+testing_bars]
            
#         early_stop = tf.keras.callbacks.EarlyStopping('val_loss', patience = patience)

#         for num, cfg in zip(range(len(config)), config):
            
#             model = build_model_LSTM(cfg[0], cfg[1], cfg[2], cfg[3], cfg[4], cfg[7], cfg[8], lag)

#             if path.exists(f'./Hyperparameter_tunning/{stock_name}/HP_Grid_Search_{i}/model_GS_{num}.h5') == True:
#                 print(f'[SECTION_1 --> id_{num}] path exits') 
#             else:
#                 model.fit( 
#                     train_X,
#                     train_y, 
...
#                     )
#                 model.save(f'./Hyperparameter_tunning/{stock_name}/HP_Grid_Search_{i}/model_GS_{num}.h5')
        
#             print(f'configuration used: {cfg}')
        
#         print(f'config DONE id_{i}')
   

#         print(train_X.shape, train_y.shape, val_X.shape, val_Y.shape, test_X.shape, test_y.shape, f"id_{i}")
        
#         K.clear_session()
#         del model
#         gc.collect()
#     return mse_best_model_train, best_indexes

# # hyperparameter tunning
# def hyperparameter_tunning_NUM(scaled_values_X, scaled_values_Y, lookback_bars, validation_bars, testing_bars, epochs, config, stock_name, patience, lag):    
...
#             if path.exists(f'./Hyperparameter_tunning/{stock_name}/HP_Grid_Search_{i}/model_GS_{num+len(config)}.h5') == True:
#                 print(f'[SECTION_1 --> id_{num}] path exits') 
#             else:
#                 model.fit( 
#                     train_X,
...
#                     callbacks=[early_stop], 
#                     )
#                 model.save(f'./Hyperparameter_tunning/{stock_name}/HP_Grid_Search_{i}/model_GS_{num+len(config)}.h5')
        
#             print(f'configuration used: {cfg}')
        
#         print(f'config DONE id_{i+len(config)}')
...
class HyperparameterTuner:
    ...
    def build_model_LSTM(self, hp):
        ...
        model.add(LSTM(units=hp_neurons, input_shape=(self.sequence_length, 3), return_sequences=True))
        model.add(Dropout(DROPOUT))
